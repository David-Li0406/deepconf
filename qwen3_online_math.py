...
# -*- coding: utf-8 -*-
"""
Warm-up + Online reasoning with dataset-level DeepConf threshold for MATH-500 (HuggingFaceH4).
...
def build_parser():
    ap = argparse.ArgumentParser("Qwen3 warm-up+online for MATH-500 (dataset-level DeepConf)")
    # model / data
    ...
    ap.add_argument("--save_pred", type=str, default="math500_dc_dataset_thr.jsonl")
...
def force_finalize_answer(model, tokenizer, text, device, args, max_new_tokens: int = 32):
    ...
    with torch.no_grad():
        ...
        ans = extract_answer_with_fallback(full)
        return ans
...
def worker_online(args, indices, part_name, device_id, stop_threshold):
    ...
    for g_idx in tqdm(indices):
        item = ds[int(g_idx)]
        problem = item["problem"]
        gt = re.sub(r'\s+|\$+', '', item["answer"]).replace("\\\\", "\\")
        ...
        pred_clean = re.sub(r'\s+|\$+', '', pred_expr or "").replace("\\\\", "\\")
        is_correct = (pred_clean == gt) or (pred_clean and (pred_clean in gt or gt in pred_clean))
                ...
                for line in fin:
                    ...
                    try:
                        ...
                        if row.get("correct") is True:
                            all_total += 1
                            all_correct += 1
                        elif row.get("gt") is not None and row.get("pred") is not None:
                            all_total += 1
                    except Exception:
                        pass
