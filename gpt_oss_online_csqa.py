...
# ---------------- CSQA-specific prompt & extractors ----------------
...
def build_messages_csqa(question, choices) -> list:
...
def build_parser():
    ap = argparse.ArgumentParser("GPT-OSS-20B warm-up+online for CSQA (dataset-level DeepConf)")
    ...
    ap.add_argument("--save_pred", type=str, default="csqa_dc_dataset_thr.gptoss.jsonl")
    return ap
...
def warmup_phase(args, tokenizer, model):
    ...
    for idx in tqdm(warm_indices, desc="Warm-up"):
        item = ds[int(idx)]
        messages = build_messages_csqa(item["question"], item["choices"])
...
def online_phase(args, tokenizer, model, stop_threshold: float):
    ...
    print(f"[CSQA] Accuracy: {acc:.4f} ({correct}/{total})")
